...
#define TEST_MODEL_FILENAME "xmm_test_forward.hmm"
#define TRAINING_FILE_PATH "/opt/usr/media/"
#define FORWARD_DATA_FILE_PATH "data/forward/"
...
//	if (true) {
		...
		path << app_get_resource_path() << FORWARD_DATA_FILE_PATH;
...
bool Hmm_Test_Forward::is_PerformingDone_child() {
	return m_analyzer->is_End();
}

double Hmm_Test_Forward::get_Probability_child() {
...
double Hmm_Test_Forward::perform_Stretching_child(const vec3 &curr_observation) {
	/// To do
...
bool Hmm_Test_Forward::reset_child() {
	m_observationCnt = 0;
	m_isPerforming = false;
	m_hmm.performance_init();
	m_analyzer->reset();
...
bool Hmm_Test_Forward::retrain_child() {
